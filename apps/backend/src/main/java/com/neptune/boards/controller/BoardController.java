...
public class BoardController {
    ...
    @Operation(
            ...
            description = "Create a Board by its UUID generated by the user"
    )
    @PostMapping(path="/{uuid}", produces = {MediaType.APPLICATION_JSON_VALUE})
    public ResponseEntity<BoardResponseDTO> createBoard(@PathVariable UUID uuid, @RequestBody BoardRequestDTO boardRequest) {
        return new ResponseEntity<>(service.createBoard(uuid, boardRequest), HttpStatus.OK);
    }

    @GetMapping(path = "/{uuid}", produces = {MediaType.APPLICATION_JSON_VALUE})
    public ResponseEntity<BoardResponseDTO> getBoard(@PathVariable UUID uuid) throws NeptuneBoardsException {
    ...
    public ResponseEntity<BoardResponseDTO> deleteBoard(@PathVariable UUID uuid) throws NeptuneBoardsException {
        return new ResponseEntity<>(service.deleteBoard(uuid), HttpStatus.OK);
    }

    @PutMapping(path="/{uuid}", produces = {MediaType.APPLICATION_JSON_VALUE})
    public ResponseEntity<BoardResponseDTO> updateBoard(@PathVariable UUID uuid, @RequestBody BoardRequestDTO boardRequest) throws NeptuneBoardsException {
